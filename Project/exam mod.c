...
void gastos_personal(struct Empleado datos_empleados[MAX_EMPLEADOS]); // NUEVA FUNCION
...
int main(){
    ...
    for (int i=0; i<MAX_EMPLEADOS;i++){ // Con este bucle nos aseguramos de que la lista de Empleado funcione correctamente
        ...
        for (int j = 0; j<MESES;j++){ //Inicializamos las horas de entrada/salida a -1 (asi en el caso de que se entre a las 00:00 funcionara correctamente)
            ...
            datos_empleados[i].dinerico[j] = 0; //REINICIAMOS LA NUEVA VARIABLE DE DINERO
    ...
    while(bucle_menu){
        ...
        switch(opcion){
            ...
            case 7:
                gastos_personal(datos_empleados);
                break;
    ...
    for (int i = 0; i < MAX_EMPLEADOS; i++) {
        if (datos_empleados[i].existe == true) {
            if (datos_empleados[i].trabajo_total[mes - 1] < 7200) { // <120 horas
                datos_empleados[i].dinerico[mes-1] = ((double) datos_empleados[i].trabajo_total[mes - 1] / 60) * 8;
            } else if (datos_empleados[i].trabajo_total[mes - 1] >= 7200 &&
                       datos_empleados[i].trabajo_total[mes - 1] < 8100) { // 120-135 horas
                datos_empleados[i].dinerico[mes-1] = ((double) datos_empleados[i].trabajo_total[mes - 1] / 60) * 12.45;
            } else if (datos_empleados[i].trabajo_total[mes - 1] >= 8100 &&
                       datos_empleados[i].trabajo_total[mes - 1] < 8700) { // 135-145 horas
                datos_empleados[i].dinerico[mes-1] = ((double) datos_empleados[i].trabajo_total[mes - 1] / 60) * 14.00;
            } else if(datos_empleados[i].trabajo_total[mes-1]>=8700) {
                datos_empleados[i].dinerico[mes-1] = ((double) datos_empleados[i].trabajo_total[mes - 1] / 60) * 15; // >145 horas
            }
            ...
            printf("%s %s:\n%.2lf euros. En todo el anio lleva %03d hora(s) y %02d minuto(s) trabajados.\n\n", datos_empleados[i].nombre,
                   datos_empleados[i].apellidos, datos_empleados[i].dinerico[mes-1], datos_empleados[i].trabajo_anio / 60,
                   datos_empleados[i].trabajo_anio % 60);

            // Reseteamos el registro del mes seleccionado: (en este caso no borramos el total de horas de mes por si se quiere acto seguido despedir al usuario, igualmente al introducir una nueva fecha de entrada y salida ya se reiniciaria).
...
void dar_de_baja(struct Empleado datos_empleados[MAX_EMPLEADOS]){
    ...
    if (coincide==false){
        ...
        datos_empleados[n].dinerico[mes-1] = (((double)datos_empleados[n].trabajo_total[mes-1]/60)*8); // Calculamos el dinero que recibira (8/h)

        printf("%s %s:\n%.2lf + plus del 3%% = %.2lf euros.\n", datos_empleados[n].nombre,datos_empleados[n].apellidos, datos_empleados[n].dinerico[mes-1], datos_empleados[n].dinerico[mes-1]*1.03);
        ...
        for (int j = 0; j<MESES;j++){
            ...
            datos_empleados[n].dinerico[j] = 0; // REINICIAMOS EL DINERICO
            for (int k = 0; k<DIAS;k++){
...
void gastos_personal(struct Empleado datos_empleados[MAX_EMPLEADOS]){
    char nombre_mes[12][20] = {"Enero","Febrero","Marzo","Abril","Mayo","Junio","Julio","Agosto","Septiembre","Octubre","Noviembre","Diciembre"};
    double gasto_mes;
    double mayor[2] = {0,0}; // En la primera posicion guardo el gasto mas bajo de un mes y en el segundo el numero del mes
    double menor[2];
    bool calcular_primer_menor = true; //Para calcular el menor necesito uno de referencia, que solo evaluo una vez

    for (int i = 0;i<MESES;i++){
        gasto_mes=0;

        for(int j = 0;j<MAX_EMPLEADOS;j++){
            gasto_mes +=  datos_empleados[j].dinerico[i];
        }
        printf("%s: %.2lf pavines.\n",nombre_mes[i],gasto_mes);

        if (calcular_primer_menor){ //Para calcular el menor necesito uno de referencia, que solo evaluo una vez
            menor[0] = gasto_mes;
            menor[1] = i;
            calcular_primer_menor = false;
        }

        if (gasto_mes > mayor[0]){
            mayor[0] = gasto_mes;
            mayor[1] = i;
        }

        if (gasto_mes < menor[0]){
            menor[0] = gasto_mes;
            menor[1] = i;
        }
    }
    printf("El mes mas caro ha sido %s.\n",nombre_mes[(int)mayor[1]]);
    printf("El mes mas barato ha sido %s.\n",nombre_mes[(int)menor[1]]);
}