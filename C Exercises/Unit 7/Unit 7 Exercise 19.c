...
int buscar (int dni, int dia, int mes, int anio) {
    for (int i=0; i<num_eventos; i++) {
        if (dni == lista[i].dni) {
            if ((dia == lista[i].f.dia) && (mes == lista[i].f.mes) && (anio == lista[i].f.anio)) {
                return i;
            }
        }
    }
...
void main(){
    ...
    puts("Ingrese el DNI del paciente que quiere buscar:");
    scanf("%d",&dni);
    puts("Ahora indique la fecha (DD/MM/AAAA)");
    scanf("%d/%d/%d",&dia,&mes,&anio);

    printf("Esta en la posicion numero %d.",buscar(dni,dia,mes,anio));
}