...
int main(){
    ...
    for (int i=0; i<MAX_EMPLEADOS;i++){ // Con este bucle nos aseguramos de que la lista de Empleado funcione correctamente
        ...
        for (int j = 0; j<MESES;j++){ //Inicializamos las horas de entrada/salida a -1 (asi en el caso de que se entre a las 00:00 funcionara correctamente)
            ...
            datos_empleados[i].dinerico[j]= 0;
...
void salario_mensual(struct Empleado datos_empleados[MAX_EMPLEADOS]) {
    ...
    for (int i = 0;i<3;i++){
        ...
        for (int j = 0;j<MAX_EMPLEADOS;j++) {
            if (datos_empleados[j].existe == true && datos_empleados[j].trabajo_total[mes - 1] > mayor &&
                j != mejores[0] &&
                j != mejores[1] &&
                j != mejores[2]) {
                mayor = datos_empleados[j].trabajo_total[mes - 1];
                mejores[i] = j;
            }
        }
    }

    printf("Los 3 mejores son:\n%s\n%s\n%s",datos_empleados[mejores[0]].nombre,datos_empleados[mejores[1]].nombre,datos_empleados[mejores[2]].nombre);
    ...
    for (int i = 0; i < MAX_EMPLEADOS; i++) {
        if (datos_empleados[i].existe == true) {
            ...
            if (mes==6 || mes==12){
                if (mes ==6){
                    for (int j = 0;j<6;j++){
                        media+= datos_empleados[i].dinerico[j];
                    }
                } else if (mes == 12){
                    for (int j = 6;j<12;j++){
                        media+= datos_empleados[i].dinerico[j];
                ...
                media/= 6;
                printf("La media de dinero del primer semestre es: %.2lf",media);
                printf("Por ello aniadiremos un 80%% del salario medio = %.2lf.\n",media + media*80/100);
            }
...
void dar_de_baja(struct Empleado datos_empleados[MAX_EMPLEADOS]){
    ...
    char codigo[10]; // Codigo es una string auxiliar en la que el usuario introducira los datos.
    bool coincide = false; // Volvemos a comprobar si el código del empleado coincide o no.
    double media,cobro; // Dinero que recibira el empleado.
    int inicio_semestre;
    ...
    if (coincide==false){
        ...
        if (mes>5){
            inicio_semestre = 6;
        } else inicio_semestre = 0;

        media = 0;
        for (int i=inicio_semestre;i<mes;i++){
            media+= datos_empleados[n].trabajo_total[i];
        }
        ...
        printf("%s %s:\n%.2lf + plus del media semestral (%.2lf) (proporcion %d/6) = %.2lf euros.\n", datos_empleados[n].nombre,datos_empleados[n].apellidos, cobro, media,mes,cobro + media*mes/6);
        ...
        for (int j = 0; j<MESES;j++){
            ...
            datos_empleados[n].dinerico[j] = 0;
            for (int k = 0; k<DIAS;k++){
